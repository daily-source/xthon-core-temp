<template>
  <div class="column is-centered slider-wrapper">
    <div class="custom-control--slider">
      <h6
        :class="{'suggested-range-color1': value > 600, 'suggested-range-color2': value >= 600 && value < 2000, 'suggested-range-color3': value >= 2000}"
        class="inputDaysAndGoal"
        style="font-weight: 100; font-style: italic;"
        v-if="printOutput">{{value | numberToUsd}}
      </h6>
      <input
        :class="{'is-circle': isCircle, 'is-small': isSmall, 'is-medium': isMedium, 'is-large': isLarge}"
...
export default {
  data() {
    return {
      value: this.goal ? this.goal : 0
    }
  },
  props: [ "step", "min", "max", "isCircle", "isSmall", "isMedium", "isLarge", "printOutput", "goal", "disabled" ],
  methods: {
    goalSelected (goal) {
      this.$emit("updateGoal", goal)
    }
  },
  watch: {
    "value" (newVal) {
      this.goalSelected (newVal)
    }
  }
}
</script>
