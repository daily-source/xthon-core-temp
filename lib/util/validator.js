...
export function validateAddress (address) {
  if (address && address.length >= 1 && address.length < 140) {
    return true
  }
  return false
}
...
export function validateTelephone (telephone) {
  ...
  var re = /^[0-9]*$/
  return (telephone.length >= 10 && telephone.length < 20 && re.test(String(telephone)))
}

export function validateNotes (notes) {
  if (notes && notes.length >= 1 && notes.length < 140) {
    return true
  }
  return false
}
...
export function validateZipCode (code) {
  if (code) {
    return code.toString().length >= 5 && code.toString().length < 10 && !isNaN(code)
  } else {
    return false
  }
}
