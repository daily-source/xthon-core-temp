...
export default {
  ...
  ADD_NEW_COMMENT: (state, { comment, count }) => {
    state.comments.data = state.comments.data.concat(comment)
    state.fundraiser.counters.commentsCount = count
  ...
  SET_ALL_FUNDRAISERS: (state, { fundraisers }) => {
    state.allFundraisers.data = fundraisers
  },
  SET_ALL_FUNDRAISERS_ADMIN: (state, { fundraisers }) => {
    state.allFundraisersAdmin.data = fundraisers
  },
  SET_ALL_CLAIMS_ADMIN: (state, { claims }) => {
    state.allClaimsAdmin.data = claims
  },
  ...
  UPDATE_FIXED_IMAGE: (state, { image, entity, target }) => {
    if (target === 'media') {
      state[entity].media.images.unshift(image)
    } else {
      state[entity].data[target] = image
    }
  },
  SAVE_INLINE_FIELD: (state, { location, value }) => {
    nestedObjectSet(state, location, value)
  },
  SAVE_FUNDRAISER_STATUS: (state, { location, value }) => {
    nestedObjectSet(state, location, value)
  },
  SAVE_CLAIM_STATUS: (state, { location, value }) => {
    nestedObjectSet(state, location, value)
  },
  REMOVE_FIXED_IMAGE: (state, { location, filename }) => {
    var entity = location.split('.')[0]
    var target = location.split('.')[location.split('.').length - 1]
    if (target === 'media') {
      var arr = state[entity]['media']['images']
      for (var i = 0; i < arr.length; i++) {
        if (arr[i] === filename) {
          arr.splice(i, 1)
        }
      }
      Vue.set(state[entity]['media'], 'images', arr)
    } else {
      Vue.set(state[entity]['data'], target, '')
    }
  },
  REMOVE_FIXED_VIDEO: (state, { location, filename }) => {
    var entity = location.split('.')[0]
    var target = location.split('.')[location.split('.').length - 1]
    if (target === 'media') {
      var arr = state[entity]['media']['videos']
      for (var i = 0; i < arr.length; i++) {
        if (arr[i] === filename) {
          arr.splice(i, 1)
        }
      }
      Vue.set(state[entity]['media'], 'videos', arr)
    } else {
      Vue.set(state[entity]['data'], target, '')
    }
  },
  ...
  ADD_VIDEO: (state, { location, youTubeID }) => {
    nestedObjectAdd(state, location, youTubeID)
  }
}
...
const nestedObjectAdd = (obj, path, value) => {
  const { lastObj, lastKey } = getNestedObject(obj, path)
  var newObj = []
  console.log(lastObj, lastKey, value)
  if (lastObj.hasOwnProperty(lastKey)) {
