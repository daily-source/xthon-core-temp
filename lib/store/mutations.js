...
export default {
  ...
  UPDATE_NONPROFIT_IMAGE: (state, { image, target }) => {
    if (target === 'media') {
      state.nonprofit.media.images.unshift(image)
    } else {
      state.nonprofit.data[target] = image
    }
  },
  ...
  REMOVE_FIXED_IMAGE: (state, { location, filename }) => {
    ...
    if (target === 'media') {
      console.log('UPDATE_NONPROFIT_IMAGE: ', entity, target)
      var arr = state[entity]['media']['images']
      console.log('new arr (BEFORE): ', arr.length)
      ...
      console.log('state.nonprofit.media.images.length', state.nonprofit.media.images.length)
    } else {
