...
export default {
  ...
  REMOVE_VIDEO: ({ commit, dispatch, state }, { location, route, youTubeID }) => {
    return new Promise((resolve, reject) => {
      removeVideo(location, route, youTubeID, state.user.tokenData.accessToken)
        .then(data => {
          commit("REMOVE_VIDEO", { location: `${location}.videos`, youTubeID: youTubeID })
          resolve(data)
        })
        .catch(err => {
          reject(err)
  ...
  ADD_VIDEO: ({ commit, dispatch, state }, { location, route, youTubeID }) => {
    return new Promise((resolve, reject) => {
      addVideo(location, route, youTubeID, state.user.tokenData.accessToken)
        .then(data => {
