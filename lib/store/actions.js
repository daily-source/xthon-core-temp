...
export default {
  ...
  SAVE_GENERIC_IMAGE: ({ commit, dispatch, state }, { image, location, route }) => {
    ...
    if (image) {
      return new Promise((resolve, reject) => {
        return requestGenericPreSignedUrl({
          ...
          .then(postedURL => {
            commit("UPDATE_NONPROFIT_IMAGE", { image: postedURL, target: target })
            resolve()
          })
          .catch(err => {
            reject(err)
  ...
  REMOVE_FIXED_IMAGE: ({ commit, dispatch, state }, { location, route }) => {
    return new Promise((resolve, reject) => {
      removeFixedImage(location, route, state.user.tokenData.accessToken)
        .then(data => {
          commit("REMOVE_FIXED_IMAGE", { location: `${location}.src` })
          resolve(data)
        })
        .catch(err => {
          reject(err)
