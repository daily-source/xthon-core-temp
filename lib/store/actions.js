import {
  ...
  postImageToS3,
  saveNonprofitLogoToBackend,
  requestNonprofitLogoPreSignedURL
} from "../api"
...
export default {
  ...
  SAVE_REPLACE_IMAGE: ({ commit, dispatch, state }, { image, location, route }) => {
    ...
    if (image) {
      return new Promise((resolve, reject) => {
        return requestNonprofitLogoPreSignedURL({
          blob: image,
          ein: route.params.ein,
          token: token
        })
          .then(preSignedURL => { return postImageToS3(preSignedURL, image) })
          .then(postedURL => { return saveNonprofitLogoToBackend({
              ein: route.params.ein,
              postedURL: postedURL,
              token: token,
              location: location
          ...
          .then(postedURL => {
            commit("UPDATE_NONPROFIT_LOGO", { image: postedURL })
            resolve()
          })
          .catch(err => {
            reject(err)
      ...
      return new Promise((resolve, reject) => {
        return saveNonprofitLogoToBackend(sub, "", token)
          .then(() => {
            commit("UPDATE_NONPROFIT_LOGO", { image: "" })
            resolve()
          })
          .catch(err => {
            reject(err)
  ...
  SAVE_PROFILE_AVATAR: ({ commit, dispatch, state }, { avatar }) => {
    ...
    if (avatar) {
      ...
      // No avatar, so save empty string (or remove the previous one)
      return new Promise((resolve, reject) => {
        return saveImageToBackend(sub, "", token)
          .then(() => {
