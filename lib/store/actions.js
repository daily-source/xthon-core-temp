...
export default {
   ...
   */
  SHARE_BY_EMAIL: ({ commit, dispatch, state }, { url, text, via, targetName, targetEmail, token }) => {
  ...
  REPORT_COMMENT: ({ commit, dispatch, state }, { commentId, userId, report, token }) => {
    return new Promise((resolve, reject) => {
      return reportComment(commentId, userId, report, token)
        .then(data => {
          return resolve(data)
        })
        .catch(err => {
