...
export function requestNonprofitLogoPreSignedURL ({ blob, token, ein }) {
  return new Promise((resolve, reject) => {
    return Vue.axios.get(
      `${baseURL}/nonprofits/${ein}/get-logo-presigned-url`,
      {
        headers: { "Authorization": `Bearer ${token}` }
      }
    ).then(data => {
...
// This functions works for the user avatar. Commented out while we finish the nonprofit method
export function saveImageToBackend (sub, postedURL, token) {
  return new Promise((resolve, reject) => {
    return Vue.axios.put(
...
export function saveNonprofitLogoToBackend ({ ein, postedURL, token, location }) {
  return new Promise((resolve, reject) => {
    return Vue.axios.put(
      `${baseURL}/nonprofits/${ein}/image`,
      {
        location: location,
        value: postedURL
      },
