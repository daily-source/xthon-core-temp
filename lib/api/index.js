...
/* This functions replaces the previous one, right?? */
...
export function removeFixedImage (location, route, token, filename) {
  return new Promise((resolve, reject) => {
    ...
    return Vue.axios.delete(
      `${baseURL}/${entity}s/${identifier}/image/${location}/${filename.substring(filename.lastIndexOf('/')+1)}`,
      {
        data: {
          fullURL: filename
        },
...
export function removeVideo (location, route, youTubeID, token) {
  ...
  console.log('removeVideo: ', route)

  return new Promise((resolve, reject) => {
    return Vue.axios.delete(
      `${baseURL}/${entity}s/${id}/video/${youTubeID}`,
      {
        headers: { "Authorization": `Bearer ${token}` }
      }
    ).then(data => {
