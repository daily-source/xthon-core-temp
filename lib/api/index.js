...
export function fetchNonprofit (ein) {
  return new Promise((resolve, reject) => {
    return Vue.axios.get(`${baseURL}/nonprofits/${ein}`)
      .then(response => {
        if (response.data.length && response.data[0].status === 'claimed') {
...
/*
 * Fetch all fundraisers. Return a 404 code if no fundraisers found.
 */
export function fetchAllFundraisers (filter, page, limit, paginated = true) {
  if (!paginated) {
    page = 1
  }
  return new Promise((resolve, reject) => {
    return Vue.axios.get(`${baseURL}/fundraisers/all?&_limit=${limit}&_page=${page++}&_filter=${filter}`)
      .then(response => {
        if (response.data.length) {
          return resolve(response.data)
        } else {
...
/*
 * Fetch all claims in admin area. Return a 404 code if no claims found.
 */
export function fetchAllClaimsAdmin (filter, token, page, limit, paginated = true) {
  if (!paginated) {
    page = 1
  }
  return new Promise((resolve, reject) => {
    return Vue.axios.get(`${baseURL}/admin/claims/all?&_limit=${limit}&_page=${page++}&_filter=${filter}`,
      {
...
/*
 * Fetch all fundraisers in admin area. Return a 404 code if no fundraisers found.
 */
export function fetchAllFundraisersAdmin (filter, token, page, limit, paginated = true) {
  if (!paginated) {
    page = 1
  }
  return new Promise((resolve, reject) => {
    return Vue.axios.get(`${baseURL}/admin/fundraisers/all?&_limit=${limit}&_page=${page++}&_filter=${filter}`,
      {
        headers: { "Authorization": `Bearer ${token}` }
      })
      .then(response => {
...
/*
 * Fetch all userss in admin area. Return a 404 code if no users found.
 */
export function fetchAllUsersAdmin (filter, token, page, limit, paginated = true) {
  if (!paginated) {
    page = 1
  }
  return new Promise((resolve, reject) => {
    return Vue.axios.get(`${baseURL}/admin/users/all?&_limit=${limit}&_page=${page++}&_filter=${filter}`,
      {
        headers: { "Authorization": `Bearer ${token}` }
      })
      .then(response => {
 ...
 * Share via email.
 */
export function shareViaEmail ({url, via, targetName, targetEmail, token}) {
  return new Promise((resolve, reject) => {
    return Vue.axios.post(
      `${baseURL}/users/services/share-via-email/`,
      {
        url: url,
        via: via,
...
/*
 * Report comment to the API.
 */
export function reportComment (commentId, userId, report, token) {
  return new Promise((resolve, reject) => {
    return Vue.axios.post(
...
/*
 * Report a nonprofit or fundraiser to the API.
 */
export function reportContent (entity, contentId, report, token) {
  return new Promise((resolve, reject) => {
    return Vue.axios.post(
      `${baseURL}/${entity}s/report/${contentId}`,
      {
        report: report
      },
      {
...
/*
 * Save a fundraiser's status.
 */
export function saveFundraiserStatus (location, value, route, token) {
  return new Promise((resolve, reject) => {
    return Vue.axios.put(
      `${baseURL}/admin/${route.name}s/${route.params.id}/update`,
      {
        location: location,
        value: value
      },
...
/*
 * Save a claim's status.
 */
export function saveClaimStatus (location, value, route, token) {
  return new Promise((resolve, reject) => {
    return Vue.axios.put(
      `${baseURL}/admin/${route.name}s/${route.params.id}/update`,
      {
...
/*
 * Save a users's account type.
 */
export function saveUserAccountType (location, value, route, token) {
  return new Promise((resolve, reject) => {
    return Vue.axios.put(
      `${baseURL}/admin/${route.name}s/${route.params.id}/update`,
      {
...
/*
 * Send a nonprofit claim form.
 */
export function submitClaimForm (ein, form, token) {
  return new Promise((resolve, reject) => {
    return Vue.axios.post(
      `${baseURL}/nonprofits/${ein}/claim`,
      {
        form: form
      },
      {
...
export function submitFundraiserForm (form, token) {
  return new Promise((resolve, reject) => {
    return Vue.axios.post(
      `${baseURL}/fundraisers/create`,
      {
        form: form
      },
      {
